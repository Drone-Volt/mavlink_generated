...
#define MAVLINK_ARDUPILOTMEGA_XML_HASH -5591196498840246705
...
typedef enum MAV_CMD
{
   ...
   MAV_CMD_OPEN_DRONE_ID_RADIO_CONTROL=12916, /* A command to control the radio of an OpenDroneID device. |Start/stop etc. the transmission of OpenDroneID radio messages from the device.| Start/stop etc. the reception of OpenDroneID radio messages by the device. Can be ignored by the device if reception is not supported.| Configures the OpenDroneID radio transmission mode.| Configures the OpenDroneID radio reception mode. Can be ignored by the device if reception is not supported.| Configures the OpenDroneID transmission period. Valid values are between 100 and 3000 ms.| Configures the WiFi channel used for transmission (in WiFi mode). If set to 0, transmission is disabled. If set to 255, it is up to the transmitter to select a WiFi channel. Valid channels are defined here: https://en.wikipedia.org/wiki/List_of_WLAN_channels. For the 2.4GHz it is 1 to 14.| Configure the region; needed for compliance tests.|  */
